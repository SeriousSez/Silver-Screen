...
{
 public enum CandidateRouteResult{Started,CandidateMissing,StageSchoolMissing,CapacityFull,AgentMissing,NavigationRejected}
 public interface ICandidateWorldRouter{CandidateRouteResult RouteToStageSchool(Candidate candidate,Action<CandidateRouteResult,int> onComplete);CandidateRouteResult RouteToExit(Candidate candidate,Action<CandidateRouteResult,int> onComplete);void ReleaseCandidate(Candidate candidate);}
 [Serializable] public sealed class RecruitmentConfiguration
 {
  public int Capacity{get;} public int ArrivalCadenceMinutes{get;} public int MaximumWaitingMinutes{get;}
  public RecruitmentConfiguration(int capacity=5,int arrivalCadenceMinutes=360,int maximumWaitingMinutes=2880){Capacity=Math.Max(1,capacity);ArrivalCadenceMinutes=Math.Max(1,arrivalCadenceMinutes);MaximumWaitingMinutes=Math.Max(60,maximumWaitingMinutes);}
 }
 public sealed class RecruitmentCoordinator:IDisposable
 {
  private readonly List<Candidate> _candidates=new List<Candidate>();private readonly ISimulationTimeService _time;private readonly CandidateGenerator _generator;private readonly ICandidateWorldRouter _router;private int _minutesUntilArrival;
  ...
  public Candidate TryGenerateArrival(){if(!HasCapacity)return null;var candidate=_generator.Generate(_time.CurrentTime);_candidates.Add(candidate);OnCandidateAdded?.Invoke(candidate);var result=_router.RouteToStageSchool(candidate,(completed,position)=>HandleArrival(candidate,completed,position));if(result!=CandidateRouteResult.Started)HandleArrival(candidate,result,-1);return candidate;}
  ...
  private void HandleMinutePassed(SimulationDateTime time){for(int i=_candidates.Count-1;i>=0;i--){var candidate=_candidates[i];if(candidate.Status!=CandidateStatus.WaitingAtStageSchool)continue;candidate.AdvanceWaitingMinute();if(candidate.WaitingMinutes>=Configuration.MaximumWaitingMinutes)Reject(candidate);}_minutesUntilArrival--;if(_minutesUntilArrival<=0){TryGenerateArrival();_minutesUntilArrival=Configuration.ArrivalCadenceMinutes;}}
  private static EmployeeRole ToEmployeeRole(ProfessionalRole role)=>role switch{ProfessionalRole.Director=>EmployeeRole.Director,ProfessionalRole.Extra=>EmployeeRole.Extra,ProfessionalRole.Crew=>EmployeeRole.Crew,_=>EmployeeRole.Actor};
